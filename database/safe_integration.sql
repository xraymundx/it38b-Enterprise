-- Safe integration script - based on fix_database.sql schema
...
SET @sql = IF(@billingrecords_exists > 0, 'CREATE TABLE IF NOT EXISTS billingrecords_backup AS SELECT * FROM billingrecords', 'SELECT "No billingrecords table" AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 1. Safely add columns to medicalrecords (using original schema from fix_database.sql)
...
-- 2. Check if foreign key constraint should be added
...
-- 3. Update the appointment status if needed
...
-- 4. Don't rename billingrecords to billing_records to maintain compatibility
-- Just add the necessary columns to billingrecords
...
-- 5. Add index on record_id for billingrecords if needed
SET @index_exists = (SELECT COUNT(*) FROM information_schema.statistics 
...
-- 6. Add foreign key from billingrecords.record_id to medicalrecords.record_id if needed
SET @fk_exists = (SELECT COUNT(*) FROM information_schema.referential_constraints 
...
-- Create a simple view for reporting (if table names match our expected schema)
SET @sql = IF(@billingrecords_exists > 0,
...
-- Completion message
SELECT 'Safe integration completed. Made compatible changes while respecting original schema.' AS message; 